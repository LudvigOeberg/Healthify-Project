...
const Simulate = {
  bloodsugar: (data, meal) => {
    ...
    for (let i = 0; i < 5; i++) {
      if (i === 0) {
        dataObjects.push({
          x: Moment(today.setHours(today.getHours(), today.getMinutes())).format('YYYY-MM-DD HH:mm'),
          y: data,
        })
      } else
        dataObjects.push({
          x: Moment(today.setHours(today.getHours() + 1, today.getMinutes())).format('YYYY-MM-DD HH:mm'),
          y: data + meal + 0.5 - i / 2,
        })
    }
    return dataObjects
  ...
  weight: (data, trainingammount, calorieintake, intensity) => {
    ...
    for (let i = 0; i < 53; i++) {
      if (i === 0) {
        dataObjects.push({
          x: Moment(today.setDate(today.getDate())).format('YYYY-MM-DD HH:mm'),
          y: data + i * ((calorieintake * 7 - 1600 * 7 - intensity * trainingammount * 400) / 7000).toFixed(2),
        })
      } else {
        dataObjects.push({
          x: Moment(today.setDate(today.getDate() + 7)).format('YYYY-MM-DD HH:mm'),
          y: data + i * ((calorieintake * 7 - 1600 * 7 - intensity * trainingammount * 400) / 7000).toFixed(2),
        })
      }
    }
