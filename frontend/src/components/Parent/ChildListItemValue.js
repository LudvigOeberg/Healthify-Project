...
function ChildListItemValue(props) {
  ...
  const weight =
    disease === 'OBESITY' && props.weights && props.weights[ehrId] && props.weights[ehrId].weight[0]
      ...
      : null
  const bloodsugar =
    disease === 'DIABETES' && props.bloodsugars && props.bloodsugars[ehrId] && props.bloodsugars[ehrId].bloodsugar
  ...
  return (
    ...
    `}
        secondary={
          props.weights && disease === 'OBESITY' && weight ? Moment(weight.time).format('YYYY-MM-DD [kl:] HH:mm') : null
        }
      />
    </ListItem>
  )
